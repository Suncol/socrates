! subversion Id for THIS file : $Id: nf_utils.mod.f 8506 2016-01-28 10:09:58Z simonc $
! $URL: http://svn-ae/ae/COMMON/stand-alone/atmos_models/socrates/tags/07.18.01/src/nf_utils.mod.f $
!-----------------------------------------------------------------------
...
!=======================================================================
     ...
     $                          attr_val )
!--------------------------------------------------------------------------
!	... Set a variable or global character attribute
!           Will place in the active netcdf file
!--------------------------------------------------------------------------
...
!--------------------------------------------------------------------------
!	... Check for arg validity; only one can exist
!--------------------------------------------------------------------------
      arg_valid(1) = PRESENT( matrix )
      arg_valid(2) = PRESENT( vector )
      arg_valid(3) = PRESENT( scalar )
...
!--------------------------------------------------------------------------
	 ...
	 write(*,'('' SET_FILE: '',a,'' File not in active list'')')
     $        filespec(:LEN_TRIM(filespec))
      end if
