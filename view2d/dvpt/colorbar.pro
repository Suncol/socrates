...
device, FILENAME='ps/colorbar.eps', /COLOR, /encaps, preview=0,  $
        font_size=12, /TIMES

COLORBAR, NColors=13, Bottom=2, Divisions=13,Format='(e9.2)'  $
      ,Range=[1.18e6,1.18e9], /vertical, /right   $
      ,Position = [0.82, 0.2, 0.9, 0.8]

device, /close   &   set_plot, 'X'   &   !p.font = -1
...
PRO COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
   ...
   RANGE=range, FONT=font, TICKLEN=ticklen, BOUND=bound, USERLEVELS=userlevels,  $
    _EXTRA=extra
...
ENDIF ELSE BEGIN
   ...
   ENDIF ELSE BEGIN
      IF N_ELEMENTS(bound) NE 0 THEN BEGIN
         tickv=INDGEN(ncolors+1)
         tickname=STRARR(ncolors+1)
         FOR i=0,ncolors DO BEGIN
            IF (i lt bound(0)) THEN tickname(i)=' ' $
            ELSE IF (i gt bound(1)) THEN tickname(i)=' ' $
            ELSE tickname(i)=STRING(form=format,userlevels(i))
         ENDFOR
         print, tickname
         PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=divisions, $
            YTICKS=1, XSTYLE=1, YSTYLE=1, TITLE=title, $
            POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
            YTICKFORMAT='(A1)', $ ;XTICKFORMAT=format,  $
