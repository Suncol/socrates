! subversion Id for THIS file : $Id: jcl.f 8506 2016-01-28 10:09:58Z simonc $
! $URL: http://svn-ae/ae/COMMON/stand-alone/atmos_models/socrates/tags/07.18.01/src/preproc/src/jcl.f $
!-----------------------------------------------------------------------
...
!-------------------------------------------------------
	 ...
	 fpth(spos:spos) = ' '
      ...
      call TIMCON( histinp(2)(:LEN_TRIM(histinp(2))), &
                   time, &
                   lout )
      call MKDATE( time, date )
      buff = ' ICDATE = ' // date // ','
      write(3,100) buff(:LEN_TRIM(buff))
      write(buff,'('' ICSEC = '',i5,'','')') INT( AMOD( time,8.64e4) )
      write(3,100) buff(:LEN_TRIM(buff))
      buff = ' /'
      write(3,100) buff(:LEN_TRIM(buff))
